...
def prepare_features(raw_data: pd.DataFrame) -> Dict[str, Any]:
    ...
    cat_cols = [col for col in X_train.select_dtypes(include=["object", "category"]).columns if col not in ("userId", "pageId")]
    encoder_mapping = {}
...
def load_train_data(storage: Optional[Storage] = None) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    logger.info("✅ [Utils] Dados carregados: X_train %s, y_train %s", X_train.shape, y_train.shape)
